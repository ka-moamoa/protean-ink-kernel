...
#define __CREATE(priority,entry)  \
        (void) __persistent_vars; /* this is to trick compiler that __persistent_vars is not an unused variable */\
        __create_thread(priority,(void *)entry,(void *)&__sram_vars,sizeof(FRAM_data_t));
#else
#define __CREATE(priority,entry)  \
        __create_thread(priority,(void *)entry,(void *)&__sram_vars[0],(void *)&__sram_vars[1],sizeof(FRAM_data_t))
#endif
