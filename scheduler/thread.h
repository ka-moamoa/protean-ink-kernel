...
#if KERNEL_FRAM  
typedef struct {
    uint8_t idx;            // index of the original buffer
    uint8_t _idx;           // index of the new buffer
    void *buf;              // holds temporary stack pointers
...
#define __shared(...)   \
        ...
        __nv uint32_t nvmBuffStartAddr; \
        static __nv FRAM_data_t __persistent_vars[2]; \
        __nv uint32_t nvmBuffEndAddr; \
...
#define __shared(...)   \
        ...
        static FRAM_data_t __sram_vars[2];
